...
function [t,data]=Read_File_dll_inp4(input_filename,p_data)
%% 读取关键词并确定关键词的数量
keyword=p_data{1};%
format_data=p_data{2};%
...
fclose(fid);
data_size=[kw_data_lines_num,kw_data_lines_end,kw_data_lines_end-kw_data_lines_num];%每个关键词对应数据行数，数据结束行号，数据开始行号；
%% 读取文件数据
...
for i=1:keyword_num
    ...
    frewind(fid1);
    mid_data=textscan(fid1,format_data{i,1},data_size(i,1),'headerlines',data_size(i,3));
...
%     mid_data2(1)=[];
    data_origin{i,2}=mid_data2;
end
fclose(fid1);
data=[p_data{1},data_origin(:,2:end)];
t=0;
end